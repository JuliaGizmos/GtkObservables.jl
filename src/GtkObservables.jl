...
Modifier flag for the SHIFT key. The `modifiers` field of a [`MouseButton`](@ref)
or [`MouseScroll`](@ref) event is a bitfield, so test for a key with
`(evt.modifiers & SHIFT) == SHIFT`. See also [`CONTROL`](@ref) and [`MOD1`](@ref).
"""
...
Modifier flag for the CONTROL key. The `modifiers` field of a [`MouseButton`](@ref)
or [`MouseScroll`](@ref) event is a bitfield, so test for a key with
`(evt.modifiers & CONTROL) == CONTROL`. See also [`SHIFT`](@ref) and [`MOD1`](@ref).
"""
...
Modifier flag for the ALT key. The `modifiers` field of a [`MouseButton`](@ref)
or [`MouseScroll`](@ref) event is a bitfield, so test for a key with
`(evt.modifiers & MOD1) == MOD1`. See also [`SHIFT`](@ref) and [`CONTROL`](@ref).
"""
...
Value of the `clicktype` field of a [`MouseButton`](@ref) event generated by
...
Value of the `direction` field of a [`MouseScroll`](@ref) event for an upward
scroll. See also [`DOWN`](@ref), [`LEFT`](@ref), and [`RIGHT`](@ref).
"""
...
Value of the `direction` field of a [`MouseScroll`](@ref) event for a downward
scroll. See also [`UP`](@ref), [`LEFT`](@ref), and [`RIGHT`](@ref).
"""
...
Value of the `direction` field of a [`MouseScroll`](@ref) event for a leftward
scroll. See also [`UP`](@ref), [`DOWN`](@ref), and [`RIGHT`](@ref).
"""
...
Value of the `direction` field of a [`MouseScroll`](@ref) event for a rightward
scroll. See also [`UP`](@ref), [`DOWN`](@ref), and [`LEFT`](@ref).
"""
const RIGHT = Gtk4.ScrollDirection_RIGHT

# Re-exports
export set_coordinates, BoundingBox, SHIFT, CONTROL, MOD1, UP, DOWN, LEFT, RIGHT,
       BUTTON_PRESS, BUTTON_RELEASE, destroy
