...
function label(value;
    ...
    if widget === nothing
        ...
        Gtk4.label(widget, value)
    end
    preserved = []
    if syncsig
        let widget=widget
            push!(preserved, on(observable; weak=true) do val
                Gtk4.label(widget, val)
            end)
        end
    end
    if own
...
function spinbutton(range::AbstractRange{T};
    ...
    observable, value = init_wobsval(Tel, observable, value; default=first(range))
    if own === nothing
        own = observable != obsin
    end
    if widget === nothing
...
function cyclicspinbutton(range::AbstractRange{T}, carry_up::Observable{Bool};
                          ...
                          own=nothing) where T
    obsin = observable
    observable, value = init_wobsval(T, observable, value; default=first(range))
    if own === nothing
        own = observable != obsin
    end
    if widget === nothing
